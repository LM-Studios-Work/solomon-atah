...
import { getDispatchBySlug, getPublishedDispatches } from '@/lib/data'
...
export function generateStaticParams() {
  return getPublishedDispatches().map((d) => ({ slug: d.slug }))
}
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
      ...
      <nav className="text-sm text-muted-foreground mb-8">
        <Link href="/dispatches" className="hover:text-foreground transition-colors">
          Dispatches
        </Link>
      ...
      <header className="mb-10 pb-8 border-b border-border">
        <div className="flex items-center gap-3 mb-4">
          {dispatch.issue && (
            <span className="text-xs font-mono text-muted-foreground">
              #{String(dispatch.issue).padStart(3, '0')}
            </span>
          )}
        ...
        <time className="block text-sm text-muted-foreground mt-4">
      ...
      {/* Content */}
      {dispatch.content && (
        <div className="prose-editorial text-muted-foreground leading-relaxed whitespace-pre-wrap">
          {dispatch.content}
        </div>
      )}

      {/* Related conversations */}
      {dispatch.relatedConversations.length > 0 && (
