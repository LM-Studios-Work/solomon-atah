...
export function CookieConsent() {
  ...
  const handleAccept = () => {
    localStorage.setItem("solomon_atah_cookie_consent", "accepted");
    // Any future analytics initialization goes here
    setIsVisible(false);
  };

  const handleReject = () => {
    localStorage.setItem("solomon_atah_cookie_consent", "rejected");
      ...
      <div className="max-w-4xl mx-auto bg-background/95 backdrop-blur-md border border-border shadow-2xl p-6 sm:p-8 flex flex-col md:flex-row items-start md:items-center justify-between gap-6 rounded-sm">
        <div className="flex-1 space-y-2">
          ...
          <p className="text-sm text-muted-foreground leading-relaxed">
            This website uses cookies and similar technologies to support functionality, enhance your browsing experience, and analyze performance. By clicking &quot;Accept All&quot;, you consent to our use of cookies as detailed in our{" "}
        ...
        <div className="flex flex-col sm:flex-row shrink-0 w-full md:w-auto mt-2 md:mt-0 gap-3">
          <button
            onClick={handleReject}
            className="w-full sm:w-auto inline-flex items-center justify-center px-6 py-3 bg-transparent text-foreground border border-border text-xs sm:text-sm font-semibold tracking-wide uppercase rounded-sm hover:bg-muted/50 transition-colors focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-foreground focus:ring-offset-background"
          >
            Reject Non-Essential
          </button>
          <button
            onClick={handleAccept}
            className="w-full sm:w-auto inline-flex items-center justify-center px-8 py-3 bg-foreground text-background text-xs sm:text-sm font-semibold tracking-wide uppercase rounded-sm hover:bg-foreground/90 transition-colors focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-foreground focus:ring-offset-background"
          >
            Accept All
          </button>
        </div>
